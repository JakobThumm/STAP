...
def load_human_animation_data(
    ...
    Args:
        animation_type (str): Type of animation to load. E.g., "HumanRobotHandover".
        human_animation_names (List[str]): List of human animation names to load.
            If None, load all animations of the given type.
        load_post_processed (bool): Whether to load the post-processed animations or the raw animations.
        verbose (bool): Whether to print out debug information. Defaults to False.

    Returns:
        List[Tuple[np.ndarray, Dict[str, Any]]]: List of tuples of the form (animation_data, animation_info).
    """
    animation_data = []

    if load_post_processed:
        animation_folder = "third_party/human-robot-animations/{}/postprocessed/point_animations/".format(
            animation_type
        )
    else:
        animation_folder = "third_party/human-robot-animations/{}/point_animations/".format(animation_type)
    info_folder = "third_party/human-robot-animations/{}/".format(animation_type)

    if human_animation_names is None:
        human_animation_names = [
            animation_name.split(".")[0]
            for animation_name in os.listdir(animation_folder)
            if animation_name.endswith(".pkl")
        ]

    for animation_name in human_animation_names:
        try:
            with open(
                animation_folder + animation_name + ".pkl",
                "rb",
            ) as pkl_file:
        ...
        try:
            with open(
                info_folder + animation_name + "_info.json",
                "r",
            ) as info_file:
...
def sin_modulation(
    ...
    """
    return (
        amplitude * np.sin((classic_animation_time - modulation_start_time) / (amplitude / speed))
...
def sample_animation_loop_properties(
    ...
    def sample_loop_properties_lists(amplitudes: List[float], speeds: List[float]) -> Tuple[List[float], List[float]]:
        return (
            ...
            [speed * random_factor_generator(std_factor=animation_info["loop_speed_std_factor"]) for speed in speeds],
        )
